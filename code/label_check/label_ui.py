...
import labeling as M
from config import load_config
from labeling import class_label_for_row, count_label_for_row

st.set_page_config(page_title="Bath Bomb Review Console", layout="wide")

# Label vocabularies come from config — the single source of truth for word sets.
_CFG = load_config()
CLASS_LABELS = _CFG["labeling"]["class_labels"]
COUNT_LABELS = _CFG["labeling"]["count_labels"]

PURE_OPTIONS = ["true", "false", "unsure"]
EXCLUDE_OPTIONS = [""] + [c for c in CLASS_LABELS if c != "pure"]
